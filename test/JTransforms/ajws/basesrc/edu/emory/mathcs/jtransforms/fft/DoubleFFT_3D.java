...
async {
		...
		final int n1d2a = n1d2;
                final int lastSlice = (l == (nthreads - 1)) ? n1d2a + 1 : firstSlice - p;
{
{
...
async {
		...
		final int ldimn2a = ldimn2;
                final int lastRow = (l == (nthreads - 1)) ? ldimn2a : firstRow + p;
{
{
...
{
                        for (int s = firstSlice; s < lastSlice; s++) {
                            int idx3a = ((slices - s) % slices) * twoSliceStride;
                            int idx5a = s * twoSliceStride;
                            for (int r = 0; r < rows; r++) {
                                int idx4a = ((rows - r) % rows) * twoRowStride;
                                int idx6a = r * twoRowStride;
                                for (int c = 1; c < columns; c += 2) {
                                    int idx1a = idx3a + idx4a + twon3 - c;
                                    int idx2a = idx5a + idx6a + c;
                                    a[idx1a] = -a[idx2a + 2];
                                    a[idx1a - 1] = a[idx2a + 1];
                                }
                            }
...
{
                        for (int s = firstSlice; s < lastSlice; s++) {
                            int idx5a = ((slices - s) % slices) * twoSliceStride;
                            int idx6a = s * twoSliceStride;
                            for (int r = 1; r < n2d2; r++) {
                                int idx4a = idx6a + (rows - r) * twoRowStride;
                                int idx1a = idx5a + r * twoRowStride + columns;
                                int idx2a = idx4a + columns;
                                int idx3a = idx4a + 1;
                                a[idx1a] = a[idx3a];
                                a[idx2a] = a[idx3a];
                                a[idx1a + 1] = -a[idx4a];
...
{
{
                        for (int s = firstSlice; s < lastSlice; s++) {
                            int idx3a = ((slices - s) % slices) * twoSliceStride;
                            int idx4a = s * twoSliceStride;
                            for (int r = 1; r < n2d2; r++) {
                                int idx1a = idx3a + (rows - r) * twoRowStride;
                                int idx2a = idx4a + r * twoRowStride;
                                a[idx1a] = a[idx2a];
                                a[idx1a + 1] = -a[idx2a + 1];
