...
/**
    ...
    private void xdft3da_subth1(final int icr, final int isgn, final double[] a, final boolean scale) {
        int nt;
        final int nthreads = Math.min(ConcurrencyUtils.WORKSTEALING_TASKS_PARTITION_SIZE, slices);
        nt = 8 * rows;
        if (columns == 4) {
        	nt >>= 1;
        } else if (columns < 4) {
    ...
    private void cdft3db_subth(final int isgn, final double[] a, final boolean scale) {
        ...
        final int nthreads = Math.min(ConcurrencyUtils.WORKSTEALING_TASKS_PARTITION_SIZE, rows);
        nt = 8 * slices;
        if (columns == 4) {
            nt >>= 1;
        } else if (columns < 4) {
