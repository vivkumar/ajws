...
public class HypervolumeArchive extends Archive {
  ...
  public boolean add(Solution solution){
    ...
    while (i < solutionsList_.size()){
      aux = (Solution) solutionsList_.get(i);            
    ...
    if (solutionsList_.size() > maxSize_) { // The archive is full
      double[][] frontValues = writeObjectivesToMatrix();
      ...
      for (i = 0; i < contributions.length; i++) {
        ...
        get(i).setCrowdingDistance(contributions[i]);
      }

      sort(new CrowdingDistanceComparator());
      
      int index = indexWorst(crowdingDistance_);
      remove(index);
    }        
    return true;
  } // add
