...
public abstract class Algorithm implements Serializable {
	...
	public int buildInitialSolSet_parallel(SolutionSet solutionSet, int populationSize,
			boolean setLocation, int evaluations) throws ClassNotFoundException, JMException {
		// Create the initial solutionSet
		final int initial_size = solutionSet.size();
		...
		evaluate_problem_parallel(solutionSet, initial_size, populationSize);

		return evaluations;
	}

	public void evaluate_problem_parallel(SolutionSet solutionSet, int initial_size, int populationSize) 
			throws ClassNotFoundException, JMException {
		finish {
			async {
				for (int i = 0; i < populationSize; i++) {
					evaluate_problem_parallel_core(i, initial_size, solutionSet, populationSize);
				}
			}
		}
	}

	private void evaluate_problem_parallel_core(int i, int initial_size, SolutionSet solutionSet, int populationSize) 
			throws ClassNotFoundException, JMException {
		Solution newSolution = solutionSet.get(i+initial_size);
