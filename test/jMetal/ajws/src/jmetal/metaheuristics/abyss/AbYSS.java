...
public class AbYSS extends Algorithm {
  ...
  public void referenceSetUpdate(boolean build) throws JMException{
    if (build) { // Build a new reference set
      ...
      solutionSet_.sort((java.util.Comparator)fitness_);
  ...
  */  
  public SolutionSet execute() throws JMException, ClassNotFoundException {
    ...
    final int initial_size = solutionSet_.size();
    Solution solution;
    ...
    // parallel
    evaluate_problem_parallel(solutionSet_, initial_size, solutionSetSize_);	// 
        
    for (int i = 0; i < solutionSetSize_; i++) {
    	solution = solutionSet_.get(i);
    	evaluations_++;
