...
public abstract class Experiment {
	...
	public void runExperiment() throws JMException, IOException {
		...
		new RunExperiment(this, 0, map_, problemList_.length).run();
	}
...
//	public abstract void algorithmSettings(String problemName, int problemId, Algorithm[] algorithm) throws ClassNotFoundException;
	...
	// Currently used only inside StandardTest.java
	// which is using Jikes RVM work-stealing
	// Other studies can also use it in same way
	 ...
	 * @throws ClassNotFoundException 
	 */
	public Algorithm algorithmSettings(String problemName, int problemIndex, int algorithm_index) throws ClassNotFoundException {
		try {
			...
			return getSettingsObject(algorithm_index, problemName, parameters);
	...
	protected Algorithm getSettingsObject(int algo_index, String problemName, HashMap parameters) {
		  String algoName = algorithmNameList_[algo_index];
		  String[] available_alogorithms = {"pNSGAII", "NSGAII", "PAES", "AbYSS", "CellDE", 
				  							"cMOEAD", "dMOPSO", "GDE3", "IBEA", "MOCell", 
				  							"MOCHC", "MOEAD", "OMOPSO", "pSMPSO", "SPEA2",
				  							"SMPSO", "SMSEMOA", "pMOEAD", "RandomSearch"};
		  int algo_type = 0;
		  for(;algo_type < available_alogorithms.length; algo_type++) {
			  if(algoName.equals(available_alogorithms[algo_type])) break;
		  }
		  
		  if(algo_type == available_alogorithms.length) {
			  System.out.println("Invalid Algorithm: "+algoName);
			  System.exit(-1);
		  }
		  ...
		  switch(algo_type) {
		  case 0: //"pNSGAII":
			  return new pNSGAII_Settings(problemName).configure(parameters);
		  case 1: //"NSGAII":
			  return new NSGAII_Settings(problemName).configure(parameters);
		  case 2: //"PAES":
			  return new PAES_Settings(problemName).configure(parameters);
		  case 3: //"AbYSS":
			  return new AbYSS_Settings(problemName).configure(parameters);
		  case 4: //"CellDE":
			  return new CellDE_Settings(problemName).configure(parameters);
		  case 5: //"cMOEAD":
			  return new cMOEAD_Settings(problemName).configure(parameters);
		  case 6: //"dMOPSO":
			  return new dMOPSO_Settings(problemName).configure(parameters);
		  case 7: //"GDE3":
			  return new GDE3_Settings(problemName).configure(parameters);
		  case 8: //"IBEA":
			  return new IBEA_Settings(problemName).configure(parameters);
		  case 9: //"MOCell":
			  return new MOCell_Settings(problemName).configure(parameters);
		  case 10: //"MOCHC":
			  return new MOCHC_Settings(problemName).configure(parameters);
		  case 11: //"MOEAD":
			  return new MOEAD_Settings(problemName).configure(parameters);
		  case 12: //"OMOPSO":
			  return new OMOPSO_Settings(problemName).configure(parameters);
		  case 13: //"pSMPSO":
			  return new pSMPSO_Settings(problemName).configure(parameters);
		  case 14: //"SPEA2":
			  return new SPEA2_Settings(problemName).configure(parameters);
		  case 15: //"SMPSO":
			  return new SMPSO_Settings(problemName).configure(parameters);
		  case 16: //"pMOEAD":
			  return new SMSEMOA_Settings(problemName).configure(parameters);
		  case 17: //"pMOEAD":
			  return new RandomSearch_Settings(problemName).configure(parameters);
		  }
		...
		for (int indicator = 0; indicator < indicatorList_.length; indicator++) {
			...
			for (int problem = 0; problem < problemList_.length; problem++) {
				...
				for (int algorithm = 0; algorithm < algorithmNameList_.length; algorithm++) {
					...
					System.out.println("Experiment.generateLatexTables: "+directory);
