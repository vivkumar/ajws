...
#	which extends/implements any class/interface with atomicsets
#	has to be translated.
# 2) Copy all the required packages: "atomicsets" and "p" (JUtils)
...
my $path_to_topPackage = "src";
my $TOP_PACKAGE = "jmetal";
my $JASTADD=$ajws . "/src";
my $path_to_classes = "bin";
## --> Add other dependencies to this classpath
# Absolute path only
my $classpaths = $ajws . "/src/AJWS.jar:" . $pwd . "/../jars/jksvm.jar:" . $pwd . "/../src/jars/junit-4.11.jar:.";
...
system("cp $pwd/tofix/pMOEAD.java $pwd/gen/jmetal/metaheuristics/moead/.");
...
sub doOperation {
  ...
  foreach (@translate_list) {
    ...
    foreach my $err_line (<ERRORS>) {
      #if($err_line =~ /error/ || $err_line =~ /Error/) {
      if((lc $err_line) =~ /error/ || (lc $err_line) =~ /Error/) {
        print "ERROR: Translation failed for File $file_name\n";
        $file_name =~ s{.*/}{}; # removes path
        my $log_file = $scratch . "/" . $file_name . ".err";
        my $mv_err = "mv " . $stderr . " " . $log_file;
print "COMMAND2 = $mv_err\n";
        system("$mv_err");
        $errors = 1;
        exit;
        last;
